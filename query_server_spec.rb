...
describe "query server normal case" do
  ...
  it "should run map funs" do
    @qs.reset!
    @qs.run(["add_fun", functions["emit-twice"]]).should == true
  ...
  describe "rereduce" do
    before(:all) do
      @fun = functions["reduce-values-sum"]
      @qs.reset!
    end
  ...
  describe "validation" do
    before(:all) do
      @fun = functions["validate-forbidden"]
  ...
  describe "ddoc list" do
      before(:all) do
        @ddoc = {
          "_id" => "foo",
          "lists" => {
            "simple" => functions["list-simple"],
            "headers" => functions["show-sends"],
            "rows" => functions["show-while-get-rows"],
            "buffer-chunks" => functions["show-while-get-rows-multi-send"],
            "chunky" => functions["list-chunky"]
          }
        }
        @qs.teach_ddoc(@ddoc)
      end
...
describe "query server that exits" do
  before(:each) do
    ...
    @ddoc = {
      "_id" => "foo",
      "lists" => {
        "capped" => functions["list-capped"],
        "raw" => functions["list-raw"]
      },
      "shows" => {
